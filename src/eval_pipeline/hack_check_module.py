...
from src.eval_pipeline.protocol import (
    add_error,
...
RULES: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "category": "security",
        "severity": "high",
        "message": "Detected shell/process invocation",
        "pattern": r"\b(os\.system|subprocess\.(run|Popen|call)|pty\.spawn)\b",
    },
    {
        "id": "network_call",
        "category": "security",
        "severity": "high",
        "message": "Detected network access call",
        "pattern": r"\b(requests\.|urllib\.|http\.client|socket\.)\b",
    },
    {
        "id": "torch_fallback",
        "category": "bypass",
        "severity": "high",
        "message": "Detected possible fallback to torch path in exception block",
        "pattern": r"except\s+[^:]*:\s*(?:.|\n){0,300}(return\s+torch\.|return\s+self\.)",
    },
    {
        "id": "silent_exception",
        "category": "bypass",
        "severity": "medium",
        "message": "Detected silent exception handling",
        "pattern": r"except\s+[^:]*:\s*(?:pass|return\s+None)",
    },
    {
        "id": "hardcoded_success",
        "category": "bypass",
        "severity": "medium",
        "message": "Detected hard-coded success marker",
        "pattern": r"print\(\s*[\"']T[\"']\s*\)|return\s+[\"']T[\"']",
    },
    {
...
def run_hack_check_module(request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if torch_metrics["suspicious_torch_op_count"] > 0:
            sev = "high" if torch_usage_policy == "forbid" else "medium"
            issues.append(
                {
        ...
        if high > 0:
            return finalize_module_result(result, start, ok=False, status="flagged_high")
        if strict and medium > 0:
            return finalize_module_result(result, start, ok=False, status="flagged_medium")
        if medium > 0 or low > 0:
            return finalize_module_result(result, start, ok=True, status="warning")
        return finalize_module_result(result, start, ok=True, status="ok")
